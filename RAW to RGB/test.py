...
def get_model_name(yaml_args):
    model_name_keyword = '%s_epoch%d_bs' % (yaml_args.name, yaml_args.Training.epochs)
...
def attatch_to_config(opt, yaml_args):
    ...
    opt.whether_save = yaml_args.Testing.whether_save
    opt.saveroot = yaml_args.Testing.saveroot
    opt.val_batch_size = 1
    opt.num_workers = 0
    opt.enable_patch = yaml_args.Testing.enable_patch
    opt.patch_size = yaml_args.Testing.patch_size
    ...
    # Dataset parameters
    opt.baseroot_test = yaml_args.Dataset.baseroot_test
    opt.baseroot_test_fusion = yaml_args.Dataset.baseroot_test_fusion
    opt.short_expo_per_pattern = yaml_args.Dataset.short_expo_per_pattern
    opt.test_option = yaml_args.Testing.test_option
...
if __name__ == "__main__":
    ...
    parser.add_argument('--yaml_path', type = str, \
        default = './options/qrnet_raw1_data4000_loss4.yaml', \
            help = 'yaml_path')
    ...
    parser.add_argument('--saveroot', type = str, default = './test_results', help = 'saving path that is a folder')
    ...
    # Dataset parameters
    parser.add_argument('--baseroot_test', type = str, \
        default = 'F:\\submitted papers\\QuadBayer Deblur\\data\\test', \
            help = 'output image baseroot')
    parser.add_argument('--baseroot_test_fusion', type = list, \
        default = ['F:\\submitted papers\\QuadBayer Deblur\\data\\test'], \
            help = 'output image baseroot')
    parser.add_argument('--short_expo_per_pattern', type = int, default = 2, help = 'the number of exposure pixel of 2*2 square')
    parser.add_argument('--test_option', type = bool, default = True, help = 'True: original image; False: fusion image')
