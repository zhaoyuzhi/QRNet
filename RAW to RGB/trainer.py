...
def Trainer(opt):
    ...
    #opt.val_batch_size = opt.val_batch_size * gpu_num
    ...
    for epoch in range(opt.epochs):
        for i, data in enumerate(train_loader):
            ...
            if opt.loss_grad_list[0] > 0 or opt.loss_grad_list[1] > 0:
                grad_loss = (opt.loss_list[0] * opt.loss_grad_list[0]) * criterion_Grad(rgbout_gen, rgbout_batch)
                fft_loss = (opt.loss_list[0] * opt.loss_grad_list[1]) * criterion_FFT(rgbout_gen, rgbout_batch)
                loss = rgb_loss + grad_loss + fft_loss
            else:
                loss = rgb_loss
            ...
            # Print log
            if opt.loss_grad_list[0] > 0 or opt.loss_grad_list[1] > 0:
                print("\r[Epoch %d/%d] [Batch %d/%d] [Full Loss: %.4f] [RGB Loss: %.4f] [Grad Loss: %.4f] [FFT Loss: %.4f] Time_left: %s" %
                    ((epoch + 1), opt.epochs, i, len(train_loader), loss.item(), rgb_loss.item(), grad_loss.item(), fft_loss.item(), time_left))
            else:
...
def Tester(opt):
    ...
    # configurations
    if opt.test_option:
        saveroot = opt.saveroot + '_original'
    else:
        saveroot = opt.saveroot + '_fusion'
    tail_name = opt.load_name.split(os.sep)[-2] # e.g., QRNet__K0.25__sigma5.0__data1111__loss11
    save_folder = os.path.join(saveroot, tail_name)
    if opt.whether_save:
        utils.check_path(save_folder)
    ...
    # Define the dataset
    if opt.test_option:
        valset = dataset.QuadBayer2RGB_Testset(opt)
    else:
        valset = dataset.QuadBayer2RGB_Testset_fusion(opt)
    print('The overall number of testing images:', len(valset))
    ...
    for i, (true_input, save_img_path) in enumerate(val_loader):
        ...
        with torch.no_grad():
            if opt.enable_patch:
                B, _, H, W = true_input.shape 
                patch_size = opt.patch_size
                assert patch_size % 4 == 0
                patchGen = utils.PatchGenerator(H, W, patch_size)
                out = torch.zeros(B, 3, H, W).float()
                for (h, w, top_padding, left_padding, bottom_padding, right_padding) in patchGen.next_patch():
                    img_patch = true_input[:, :, h:h+patch_size, w:w+patch_size]
                    sample = generator(img_patch)
                    out_patch = sample['dec_rgb']
        ...
        save_full_path = os.path.join(save_folder, save_img_path)
        print('%d-th image is saved at: %s' % (i + 1, save_full_path))
