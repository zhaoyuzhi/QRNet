...
class FFTLoss(nn.Module):
    ...
    def forward(self, x, y):
        if IS_HIGH_VERSION:
            x_fft1 = torch.fft.fft2(x, dim = (-2, -1))
            x_fft1 = torch.stack((x_fft1.real, x_fft1.imag), dim = -1)
            y_fft1 = torch.fft.fft2(y, dim = (-2, -1))
            y_fft1 = torch.stack((y_fft1.real, y_fft1.imag), dim = -1)
        else:
