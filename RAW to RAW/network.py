...
'''
class QRNet1(nn.Module):
    def __init__(self, opt):
        super(QRNet1, self).__init__()
        ...
        self.enc_rgb_3 = Conv2dLayer(opt.start_channels * 2, opt.start_channels * 4, 3, 2, 1, pad_type = opt.pad, norm = opt.norm)
        self.enc_rgb_3_2 = Conv2dLayer(opt.start_channels * 4, opt.start_channels * 4, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        
        # Bottleneck
        self.bottleneck_rgb = nn.Sequential(
    ...
    def forward(self, x):
        ...
        # enc3
        enc_rgb_3 = self.enc_rgb_3(enc_rgb_2)                   # out: batch * 256 * 16 * 16
        enc_rgb_3 = self.enc_rgb_3_2(enc_rgb_3)                 # out: batch * 256 * 16 * 16

        # bottleneck
        bot = self.bottleneck_rgb(enc_rgb_3)                    # out: batch * 256 * 16 * 16

        # dec1
        bot = torch.cat((bot, enc_rgb_3), 1)                    # out: batch * 512 * 16 * 16
        dec_rgb = self.dec_rgb_1(bot)                           # out: batch * 128 * 32 * 32

        # dec2
        dec_rgb = torch.cat((dec_rgb, enc_rgb_2), 1)            # out: batch * 256 * 32 * 32
        dec_rgb = self.dec_rgb_2(dec_rgb)                       # out: batch * 64 * 64 * 64

        # dec3
        dec_rgb = torch.cat((dec_rgb, enc_rgb_1), 1)            # out: batch * 128 * 64 * 64
...
# ----------------------------------------
#      Generator for ablation study
# ----------------------------------------
class QRNet_ab3(nn.Module):
    def __init__(self, opt):
        super(QRNet_ab3, self).__init__()
    ...
    def forward(self, x):
        ...
        # enc1
        enc_rgb_1 = self.enc_rgb_1(x_psdown4)                   # out: batch * 64 * 64 * 64
...
class QRNet_ab4(nn.Module):
    def __init__(self, opt):
        super(QRNet_ab4, self).__init__()
        ...
        self.bottleneck_cat_2 = Conv2dLayer(opt.start_channels * 2, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        self.bottleneck_rgb_2_1 = nn.Sequential(
            ResConv2dLayer(opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm),
            ResConv2dLayer(opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        )
        ...
        self.bottleneck_cat_3 = Conv2dLayer(opt.start_channels, opt.start_channels, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        self.bottleneck_rgb_3_1 = ResConv2dLayer(opt.start_channels, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        self.bottleneck_rgb_3_2 = ResConv2dLayer(opt.start_channels, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
    ...
    def forward(self, x):
        ...
        # bottleneck
        #bot2 = torch.cat((enc_rgb_2, bot1_up), 1)               # out: batch * 384 * 32 * 32
        bot2 = self.bottleneck_cat_2(enc_rgb_2)                 # out: batch * 128 * 32 * 32
        bot2 = self.bottleneck_rgb_2_1(bot2)                    # out: batch * 128 * 32 * 32
        #bot2_up = F.interpolate(bot2, scale_factor = 2, mode = 'bilinear')
        bot2 = self.bottleneck_rgb_2_2(bot2)                    # out: batch * 128 * 32 * 32
        # concatenate and conv
        ...
        # bottleneck
        #bot3 = torch.cat((enc_rgb_1, bot2_up), 1)               # out: batch * 192 * 64 * 64
        bot3 = self.bottleneck_cat_3(enc_rgb_1)                 # out: batch * 64 * 64 * 64
        bot3 = self.bottleneck_rgb_3_1(bot3)                    # out: batch * 64 * 64 * 64
        bot3 = self.bottleneck_rgb_3_2(bot3)                    # out: batch * 64 * 64 * 64
        # concatenate and conv
        dec_rgb = torch.cat((dec_rgb, bot3), 1)                 # out: batch * 128 * 64 * 64
...
class QRNet_ab5(nn.Module):
    def __init__(self, opt):
        super(QRNet_ab5, self).__init__()
        ...
        # Decoder rgb
        self.dec_rgb_1 = TransposeConv2dLayer(opt.start_channels * 8, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm, scale_factor = 2)
        self.dec_rgb_2 = TransposeConv2dLayer(opt.start_channels * 2, opt.start_channels, 3, 1, 1, pad_type = opt.pad, norm = opt.norm, scale_factor = 2)
        self.dec_rgb_3 = Conv2dLayer(opt.start_channels * 2, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        self.dec_rgb_4 = Conv2dLayer(opt.start_channels // 4 * 2 * 2, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        self.dec_rgb_5 = Conv2dLayer(opt.start_channels // 4 * 2 * 2, opt.out_channels, 3, 1, 1, pad_type = opt.pad, norm = 'none', activation = 'none')
    ...
    def forward(self, x):
        ...
        # concatenate and conv
        #dec_rgb = torch.cat((dec_rgb, bot2), 1)                 # out: batch * 256 * 32 * 32
        dec_rgb = self.dec_rgb_2(bot2)                          # out: batch * 64 * 64 * 64
...
class QRNet_ab6(nn.Module):
    def __init__(self, opt):
        super(QRNet_ab6, self).__init__()
        ...
        self.dec_rgb_1 = TransposeConv2dLayer(opt.start_channels * 8, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm, scale_factor = 2)
        self.dec_rgb_2 = TransposeConv2dLayer(opt.start_channels * 2, opt.start_channels, 3, 1, 1, pad_type = opt.pad, norm = opt.norm, scale_factor = 2)
        self.dec_rgb_3 = Conv2dLayer(opt.start_channels, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        self.dec_rgb_4 = Conv2dLayer(opt.start_channels // 4 * 2 * 2, opt.start_channels * 2, 3, 1, 1, pad_type = opt.pad, norm = opt.norm)
        self.dec_rgb_5 = Conv2dLayer(opt.start_channels // 4 * 2 * 2, opt.out_channels, 3, 1, 1, pad_type = opt.pad, norm = 'none', activation = 'none')
    ...
    def forward(self, x):
        ...
        # concatenate and conv
        #dec_rgb = torch.cat((dec_rgb, bot3), 1)                 # out: batch * 128 * 64 * 64
        dec_rgb = self.dec_rgb_3(bot3)                          # out: batch * 128 * 64 * 64
