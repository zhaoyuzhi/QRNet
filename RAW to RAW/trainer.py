...
def Trainer(opt):
    ...
    for epoch in range(opt.epochs):
        for i, data in enumerate(train_loader):
            ...
            rawout_batch = data['rawout_batch'].cuda()

            # Process patch
            if len(input_batch.shape) == 5:
                _, _, _, H, W = input_batch.shape # B, B, C, H, W
                input_batch = input_batch.view(-1, 1, H, W)
                rawout_batch = rawout_batch.view(-1, 1, H, W)
            
            # Train Generator
            optimizer_G.zero_grad()
            sample = generator(input_batch)
            rawout_gen = sample['dec_rgb']

            # Compute losses
            loss = opt.loss_list[0] * criterion_L1(rawout_gen, rawout_batch)
            ...
            # Print log
            print("\r[Epoch %d/%d] [Batch %d/%d] [Full Loss: %.4f] Time_left: %s" %
        ...
        if (epoch + 1) % 1 == 0:
            img_list = [input_batch, rawout_gen, rawout_batch]
